...
bash dev.sh up             # start the full stack (run from your macOS terminal)
bash dev.sh -h             # list all available dev.sh commands
bash dev.sh help           # full developer quick reference
```
...
**Two ways to run locally** — `bash dev.sh up` (port 8000) is for day-to-day development. `bash dev.sh run` (port 8080) provisions a local Kubernetes cluster and is only needed when validating the CI/CD pipeline end-to-end; most developers will never need it.
